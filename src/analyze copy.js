        ...
        case !isColumns && !isFields:
            ...
            if (columns !== null && columns !== undefined) {
                ...
                if (columnHeaders.length) {
                    ...
                    console.log({ joinedHeads, joinedRows, expandedData })
                    ...
                    if (searchHeaderRow == null || searchHeaderRow === undefined) {
                        ...
                        console.log({ columnHeads, columnCells })
                            ...
                            if (pose !== null && pose !== undefined) {
                                ...
                                if (colOffset !== colIndex) {
                                    ...
                                    const check = columnHeads
                                        ...
                                        .join('|')
                                    console.log({ headIndex, check })
                                    let window = []
                                    let windowCnt = colIndex
                                    do {
                                        window = columnCells
                                            .filter(function (cell, index) {
            ...
            if (fields !== null && fields !== undefined) {
                ...
                for (let fCnt = 0; fCnt < fields.length; fCnt++) {
                    ...
                    if (header !== null && header !== undefined) {
                        ...
                        // left | header| right  0
                        //		| bottom|		+1
                        // rules hold the calculation values necessary
                        // to determine the offset between the found and the expected position
                        // rules with an index below 2 are considered as columns
                        // rules with an index above 1 are considered as rows
                        ...
                        const headerCell = data[field.row - 1][field.col - 1]
                        const window = [right, left, bottom, top, headerCell].map((c) => (typeof c === 'string' ? c : ''))
                        const rules = [1, -1, 1, -1, 0]
                        ...
                        if (searchHeader === null || searchHeader === undefined) {
                            ...
                            // something was found
                            const { refIndex } = searchHeader
                            const rule = rules[refIndex]
                            if (rule !== 0) {
                                if (refIndex > 1) {
                                    errors.push(new IncorrectRowIndex(filename, sheetName, field.key, field.row + rule))
                                } else {
                                    errors.push(new IncorrectColumnIndex(filename, sheetName, field.key, field.col + rule))
                                }
                            }
                            if (searchHeader.score >= 0.001) {
